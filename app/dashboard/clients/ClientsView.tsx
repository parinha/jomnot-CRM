...
const fmt = fmtUSD
const EMPTY_FORM: Omit<Client, 'id'> = { name: '', contactPerson: '', phone: '', address: '', email: '', note: '' }
...
export default function ClientsView() {
  ...
  function openEdit(client: Client) {
    ...
    setForm({ name: client.name, contactPerson: client.contactPerson ?? '', phone: client.phone, address: client.address, email: client.email, note: client.note ?? '' })
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50">
                ...
                <th className="text-right px-4 py-3 font-medium text-zinc-500 hidden sm:table-cell">Remaining</th>
        ...
        <ModalShell onClose={closeModal}>
          <div className="p-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <FormField label="Note" id="note" value={form.note ?? ''} onChange={(v) => setForm((p) => ({ ...p, note: v }))} placeholder="Internal notes about this client…" textarea />
            </div>
