      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {(Object.keys(PROJECT_STATUS_CONFIG) as ProjectStatus[]).map((s) => {
          ...
          return (
            ...
            >
              <p
                className={`text-2xl font-bold ${statusFilter === s ? 'text-white' : 'text-zinc-900'}`}
              >
                {count}
              </p>
              <p
                className={`text-xs mt-0.5 ${statusFilter === s ? 'text-zinc-300' : 'text-zinc-500'}`}
              >
                {cfg.label}
              </p>
            </button>
            ...
            <tbody>
              {paged.map((project, i) => {
                ...
                return (
                  <tr
                    key={project.id}
                    className={`border-b border-zinc-100 last:border-0 hover:bg-zinc-50/60 transition ${i % 2 === 1 ? 'bg-zinc-50/40' : ''}`}
                  >
                          ...
                          {linkedInvoices.map(
                            (inv) =>
                              inv && (
                                <span
                                  key={inv.id}
                                  className="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-zinc-100 text-zinc-600"
                                >
                                  {inv.number}
                                </span>
                              )
          ...
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-start justify-between px-6 py-4 border-b border-zinc-200 shrink-0">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${PROJECT_STATUS_CONFIG[detailProject.status].cls}`}
                >
                ...
                <button
                  ...
                  className="text-zinc-400 hover:text-zinc-700 transition ml-1"
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
                  ...
                  <div className="flex flex-col gap-2">
                    {detailProject.items.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="flex items-center gap-3 p-3 rounded-lg border border-zinc-100 hover:border-zinc-200 transition"
                        >
                          <button
                          ...
                          <span
                            className={`text-sm flex-1 ${item.status === 'done' ? 'line-through text-zinc-400' : 'text-zinc-800'}`}
                          >
                            {item.description}
                          </span>
                        </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {form.items.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 p-2.5 rounded-lg border border-zinc-100 group"
                    >
                      <button
