    ...
    <div className="min-h-screen flex bg-zinc-100">
      ...
      <aside className={[
        ...
        <nav className="flex-1 p-3 flex flex-col gap-0.5 overflow-y-auto">
          ...
          <NavItem href="/dashboard/payments" label="Payments" onClick={close} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" pathname={pathname} />
        ...
        <div className="p-3 border-t border-zinc-100 shrink-0 flex flex-col gap-0.5">
          ...
          <button
            onClick={async () => { close(); await signOut(); router.replace('/login') }}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 md:h-16 bg-white border-b border-zinc-200 flex items-center px-4 md:px-6 gap-3 shrink-0 sticky top-0 z-30">
          ...
          <div ref={searchRef} className="relative flex-1 max-w-md">
            ...
            {query && (
              <button onClick={() => { setQuery(''); setShowResults(false) }} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-600">
                ...
                {matchedClients.length === 0 && matchedInvoices.length === 0 ? (
                  ...
                  <>
                    {matchedClients.length > 0 && (
                      <div>
                        <p className="px-4 py-2 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider border-b border-zinc-100">Clients</p>
                        {matchedClients.map((c) => (
                          <button key={c.id} onClick={() => go('/dashboard/clients')} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 transition text-left">
                    ...
                    {matchedInvoices.length > 0 && (
                      <div className={matchedClients.length > 0 ? 'border-t border-zinc-100' : ''}>
                        <p className="px-4 py-2 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider border-b border-zinc-100">Invoices</p>
                        {matchedInvoices.map((inv) => {
                          ...
                          return (
                            <button key={inv.id} onClick={() => go('/dashboard/invoices')} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 transition text-left">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-zinc-400 truncate">{client?.name ?? '—'} · {fmtUSD(total)}</p>
                              </div>
                            </button>
                          )
                        })}
