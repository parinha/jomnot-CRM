...
import { useStore, type Project, type ProjectPhases } from '../AppStore';
...
function getProjectCol(project: Project): string {
  ...
  return 'delivered'; // all phases done but not marked complete
}
...
function getPhasesForCol(col: string): ProjectPhases {
  ...
  // Mark all phases before this column as done
  for (let i = 0; i < colIdx; i++) {
    phases[PHASE_ORDER[i]] = true;
  }
  // 'delivered' and 'completed' cols mean all phases are done
...
function sortCards(cards: Project[], today: string): Project[] {
  return [...cards].sort((a, b) => {
    ...
    // Both late or both not late: sort by due date ascending (earliest first), no date last
...
export default function KanbanView() {
  const { projects, clients, setProjects } = useStore();
                      ...
                      >
                        ...
                        {inDelivered && (
                          <button
                            ...
                            className="mt-2.5 w-full h-7 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 text-[11px] font-semibold hover:bg-emerald-500/25 transition"
                          >
                            Mark as Completed
                          </button>
                        )}
