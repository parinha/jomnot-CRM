...
export default function InvoicePrint({ id }: { id: string }) {
  ...
  if (notFound) return (
    <div className="min-h-screen flex items-center justify-center text-zinc-500">Invoice not found.</div>
  )
  if (!data) return (
    <div className="min-h-screen flex items-center justify-center text-zinc-400 text-sm">Loading…</div>
  )

  const { invoice, client, company, payment } = data
  const subtotal      = invoice.items.reduce((s, it) => s + it.qty * it.unitPrice, 0)
  const depositAmount = invoice.depositPercent != null ? subtotal * (invoice.depositPercent / 100) : null
  ...
  return (
    <>
      ...
      <div className="no-print pt-14 min-h-screen bg-zinc-200 flex justify-center py-10 px-4">
        <Sheet invoice={invoice} client={client} company={company} payment={payment}
          subtotal={subtotal} depositAmount={depositAmount} balanceDue={balanceDue} />
      </div>
...
function Sheet({
  invoice, client, company, payment, subtotal, depositAmount, balanceDue,
}: {
  invoice:       Invoice
  client:        Client | null
  company:       CompanyProfile
  ...
  const hasPayment   = payment.bankName || payment.accountName || payment.accountNumber || payment.swiftCode
    ...
    >
      ...
      {invoice.status === 'draft' && (
        <div style={{
          ...
          <span style={{
            fontSize: '88px', fontWeight: 900, color: 'rgba(0,0,0,0.05)',
            textTransform: 'uppercase', transform: 'rotate(-30deg)',
      ...
      <div style={{ padding: '14mm 14mm 10mm', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ maxWidth: '55%' }}>
            {company.logo ? (
              ...
              <img src={company.logo} alt="Logo"
                style={{ height: '52px', maxWidth: '140px', objectFit: 'contain', display: 'block', marginBottom: '10px' }} />
            ) : null}
            {company.name && (
              <p style={{ fontWeight: 700, fontSize: '12px', marginBottom: '3px' }}>{company.name}</p>
            )}
            {company.address && (
              <p style={{ color: MUTED, whiteSpace: 'pre-wrap', marginBottom: '2px' }}>{company.address}</p>
            )}
            {company.phone && (
              <p style={{ color: MUTED }}>{company.phone}</p>
            )}
          </div>

          {/* Right — INVOICE title + meta */}
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <p style={{ fontSize: '28px', fontWeight: 800, letterSpacing: '-0.5px', marginBottom: '10px', color: DARK }}>
              INVOICE
            </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '4px' }}>
          ...
          <thead>
            <tr style={{ backgroundColor: BRAND }}>
              <th style={{ textAlign: 'center', padding: '8px 6px', color: DARK, fontWeight: 700, fontSize: '10px' }}>
                No.
              </th>
              <th style={{ textAlign: 'left', padding: '8px 10px', color: DARK, fontWeight: 700, fontSize: '10px' }}>
                Description / Scope of Work
              </th>
              <th style={{ textAlign: 'center', padding: '8px 10px', color: DARK, fontWeight: 700, fontSize: '10px' }}>
                Qty
              </th>
              <th style={{ textAlign: 'right', padding: '8px 10px', color: DARK, fontWeight: 700, fontSize: '10px' }}>
                Unit Price
              </th>
              <th style={{ textAlign: 'right', padding: '8px 10px', color: DARK, fontWeight: 700, fontSize: '10px' }}>
                Total
          ...
          <tbody>
            {invoice.items.map((item, i) => (
              <tr key={item.id} style={{ borderBottom: `1px solid ${LIGHT}` }}>
                <td style={{ padding: '9px 6px', textAlign: 'center', verticalAlign: 'top', color: MUTED, fontSize: '10px' }}>
                  {i + 1}
                </td>
                <td style={{ padding: '9px 10px', verticalAlign: 'top', wordBreak: 'break-word', paddingRight: '12px' }}>
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {scope && (
                          <p style={{ marginTop: '4px', color: MUTED, fontSize: '10px', whiteSpace: 'pre-wrap', lineHeight: '1.7' }}>{scope}</p>
                        )}
                      </>
                    )
                  })()}
                </td>
                <td style={{ padding: '9px 10px', textAlign: 'center', verticalAlign: 'top', color: MUTED }}>
                  {item.qty}
                </td>
                <td style={{ padding: '9px 10px', textAlign: 'right', verticalAlign: 'top', color: MUTED }}>
                  {fmt(item.unitPrice)}
                </td>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '24px' }}>
          <div style={{ minWidth: '220px', backgroundColor: '#e8e8e8', borderRadius: '3px', overflow: 'hidden' }}>
            {/* Subtotal */}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 12px', borderBottom: `1px solid ${LIGHT}`, gap: '40px' }}>
              <span style={{ color: MUTED }}>Subtotal</span>
              <span style={{ fontWeight: 600 }}>{fmt(subtotal)}</span>
            </div>
            {/* Deposit row */}
            {depositAmount != null && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 12px', borderBottom: `1px solid ${LIGHT}`, gap: '40px', color: '#166534' }}>
                <span>Deposit ({invoice.depositPercent}%)</span>
                <span style={{ fontWeight: 600 }}>({fmt(depositAmount)})</span>
              </div>
            )}
            {/* Grand Total / Balance Due */}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', gap: '40px', borderTop: `2px solid ${DARK}` }}>
              <span style={{ fontWeight: 700, fontSize: '12px' }}>
                {depositAmount != null ? 'Balance Due' : 'Grand Total'}
              </span>
              <span style={{ fontWeight: 800, fontSize: '12px' }}>
                {fmt(balanceDue ?? subtotal)}
              </span>
            </div>
          </div>
        </div>
        ...
        {invoice.notes && (
          <div style={{ marginBottom: '16px' }}>
            <p style={{ fontSize: '9px', fontWeight: 700, color: MUTED, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '4px' }}>Notes</p>
            <p style={{ color: MUTED, whiteSpace: 'pre-wrap', fontSize: '10.5px' }}>{invoice.notes}</p>
          </div>
        )}

        {/* ═══ USAGE RIGHTS ═══ */}
        <div style={{
          border: `1px solid ${LIGHT}`, borderRadius: '4px',
          ...
          <p style={{ color: MUTED, fontSize: '10px', lineHeight: '1.6' }}>
            The work delivered under this invoice is <strong>non-exclusive</strong>. The client may use the materials
            within the agreed scope of work for publishing and promotional boosting for a period of{' '}
            <strong>3 months</strong> from the invoice date. Any usage, re-use, or boosting beyond this period
            will require an additional service fee.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', marginBottom: '28px' }}>
          ...
          <div style={{ paddingRight: '24px' }}>
            <p style={{ fontSize: '10.5px', color: MUTED, marginBottom: '0' }}>Customer&apos;s signature</p>
          ...
          <div style={{ paddingLeft: '24px', borderLeft: `1px solid ${LIGHT}` }}>
            <p style={{ fontSize: '10.5px', color: MUTED, marginBottom: '0' }}>Seller&apos;s signature</p>
            ...
            <div style={{ borderTop: `1.5px solid ${DARK}`, width: '160px', paddingTop: '5px' }}>
              <p style={{ fontSize: '10.5px', fontWeight: company.signatoryName ? 600 : 400, color: company.signatoryName ? DARK : MUTED }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <p style={{
              fontSize: '9px', fontWeight: 700, letterSpacing: '0.1em',
              textTransform: 'uppercase', color: DARK, marginBottom: '10px',
            }}>
              Payment Information
            </p>
            {hasPayment ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                {payment.accountNumber && <PayRow label="Account Number" value={payment.accountNumber} />}
              ...
              <p style={{ color: MUTED, fontStyle: 'italic', fontSize: '10px' }}>No payment information set.</p>
            )}
          </div>
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <tr>
      <td style={{ paddingRight: '16px', paddingBottom: '3px', color: MUTED, textAlign: 'right', fontSize: '11px' }}>
        {label}
      </td>
      <td style={{ paddingBottom: '3px', fontWeight: 700, textAlign: 'right', fontSize: '11px', whiteSpace: 'nowrap' }}>
        {value}
      </td>
    </tr>
  )
...
function PayRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', gap: '8px', fontSize: '11px', lineHeight: '1.5', alignItems: 'baseline' }}>
