...
export default function SortTh({ col, active, dir, onSort, className, children }: SortThProps) {
  ...
  return (
    <th
      onClick={() => onSort(col)}
      className={`font-medium text-zinc-500 cursor-pointer select-none hover:text-zinc-800 transition ${className ?? ''}`}
    >
      <span className="inline-flex items-center gap-1">
        {children}
        <span className={isActive ? 'text-zinc-700' : 'text-zinc-300'}>
          {isActive ? (
            dir === 'asc'
            ...
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 9l4-4 4 4M8 15l4 4 4-4" /></svg>
          )}
        </span>
      </span>
    </th>
