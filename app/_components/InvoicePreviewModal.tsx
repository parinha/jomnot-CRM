...
export default function InvoicePreviewModal({ invId, onClose }: { invId: string; onClose: () => void }) {
      ...
      <div className="flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-zinc-200 shrink-0">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-0.5">{client?.name ?? '—'} · {inv.date}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-1 text-sm">
            <div><span className="text-zinc-400 text-xs block">Payment Terms</span><span className="text-zinc-700">{inv.paymentTerms ?? 'Due on receipt'}</span></div>
            {client?.email && <div><span className="text-zinc-400 text-xs block">Email</span><span className="text-zinc-700">{client.email}</span></div>}
            {client?.phone && <div><span className="text-zinc-400 text-xs block">Phone</span><span className="text-zinc-700">{client.phone}</span></div>}
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b border-zinc-200">
                <tr>
                  ...
                  <th className="text-center px-3 py-2 text-xs font-medium text-zinc-500 w-12">Qty</th>
                  <th className="text-right px-3 py-2 text-xs font-medium text-zinc-500 w-24 hidden sm:table-cell">Unit Price</th>
                  <th className="text-right px-3 py-2 text-xs font-medium text-zinc-500 w-24">Total</th>
                </tr>
              </thead>
              <tbody>
                {inv.items.map((item, i) => (
                  <tr key={item.id} className={`border-b border-zinc-100 last:border-0 ${i % 2 === 1 ? 'bg-zinc-50/40' : ''}`}>
                    <td className="px-3 py-2.5 text-zinc-800">{item.description}</td>
                    <td className="px-3 py-2.5 text-center text-zinc-500">{item.qty}</td>
                    <td className="px-3 py-2.5 text-right text-zinc-500 hidden sm:table-cell">{fmt(item.unitPrice)}</td>
                    <td className="px-3 py-2.5 text-right font-medium text-zinc-900">{fmt(item.qty * item.unitPrice)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
